 ...
 const generationRandomMatrix = (lato) => {
     const newMatrix = [];
     for (let x = 0; x < lato; x++) {
         newMatrix[x] = [];
         for (let y = 0; y < lato; y++) {
 ...
 function simulation(x, y, z, matrix, lato) {
     let numberAlive = 0;
 ...
 function currentPoint(matrix, index, lato) {
 ...
 //     return newMatrix
 // }
 function simulationCustomRule(x, y, z, matrix, lato, customRule) {
 ...
 function runSimulation(lato, Matrix, customRule) {
     const { underpopulated, stable, birth, overpopulated, neigh } = customRule;
     let newMatrix = []
     for (let x = 0; x < lato; x++) {
         newMatrix[x] = [];
 ...
 onmessage = function(event) {
     const { x, y, z, Matrix, lato, Rule } = event.data;
     // console.log("SONO UN EVENTO BELLISSIMIO", event.data)
     let result = simulationCustomRule(x, y, z, Matrix, lato, Rule);
     console.log("RISUOLTOTA", result)
     postMessage({ result, x, y, z });
 }